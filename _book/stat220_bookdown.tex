...
# A tibble: 24 x 5
   ...
   <int> <chr> <chr>         <dbl> <fct>    
 1     2 9th   Asia             63 9th      
 2    13 9th   Middle East      78 9th      
 3    14 9th   Middle East      50 9th      
 4    19 9th   Africa           99 9th      
 5    21 9th   Asia             99 9th      
 6    23 9th   South America    55 9th      
 7     5 10th  Asia             68 10th     
 8     6 10th  Africa           68 10th     
 9     7 10th  Middle East      75 10th     
10     8 10th  Africa           60 10th     
11     9 10th  Asia             95 10th     
12    10 10th  North America    74 10th     
13    11 10th  Europe           69 10th     
14    16 10th  Africa           82 10th     
15    17 10th  North America    52 10th     
16    18 10th  Africa           96 10th     
17    20 10th  Asia             61 10th     
18    24 10th  South America    92 10th     
19     1 11th  North America    89 11th     
20     3 11th  Europe           98 11th     
21     4 11th  South America    76 11th     
22    12 11th  North America    69 11th     
23    15 11th  North America    93 11th     
24    22 11th  North America    66 11th     
\end{verbatim}
...
\hline
2018 & Green Book & A working-class Italian-American bouncer becomes the driver for an African-American classical pianist on a tour of venues through the 1960s American South. & 130 & 8.2 & 519143\\
\hline
...
\hline
2007 & No Country for Old Men & Violence and mayhem ensue after a hunter stumbles upon a drug deal gone wrong and more than two million dollars in cash near the Rio Grande. & 122 & 8.2 & 1001648\\
\hline
...
\hline
1994 & Forrest Gump & The presidencies of Kennedy and Johnson, the Vietnam War, the Watergate scandal and other historical events unfold from the perspective of an Alabama man with an IQ of 75, whose only desire is to be reunited with his childhood sweetheart. & 142 & 8.8 & 2136115\\
\hline
...
\hline
1991 & The Silence of the Lambs & A young F.B.I. cadet must receive the help of an incarcerated and manipulative cannibal killer to help catch another serial killer, a madman who skins his victims. & 118 & 8.6 & 1467541\\
\hline
  ...
  \FunctionTok{sidebarLayout}\NormalTok{(}
    \FunctionTok{sidebarPanel}\NormalTok{(}
      ...
      \FunctionTok{radioButtons}\NormalTok{(}\StringTok{"yearInput"}\NormalTok{, }\StringTok{"Year"}\NormalTok{,}
                  ...
                  \AttributeTok{selected =} \StringTok{"2022"}\NormalTok{),}
...
\FunctionTok{shinyApp}\NormalTok{(}\AttributeTok{ui =}\NormalTok{ ui, }\AttributeTok{server =}\NormalTok{ server, }\AttributeTok{options =} \FunctionTok{list}\NormalTok{(}\AttributeTok{height =} \DecValTok{800}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\hypertarget{add-placeholders-for-outputs}{%
\subsection{Add placeholders for outputs}\label{add-placeholders-for-outputs}}
...
\NormalTok{    ),}
    \FunctionTok{mainPanel}\NormalTok{(}
      \FunctionTok{plotOutput}\NormalTok{(}\StringTok{"coolplot"}\NormalTok{),}
      \FunctionTok{br}\NormalTok{(), }\FunctionTok{br}\NormalTok{(),}
      \FunctionTok{tableOutput}\NormalTok{(}\StringTok{"results"}\NormalTok{)}
\NormalTok{    )}
\NormalTok{  )}
\NormalTok{)}
...
\texttt{Server} function will be responsible for listening to changes to the inputs and creating outputs to show in the app.

\hypertarget{implementation-1}{%
\subsection{Implementation 1}\label{implementation-1}}
...
\NormalTok{ui }\OtherTok{\textless{}{-}} \FunctionTok{fluidPage}\NormalTok{(}
  \FunctionTok{numericInput}\NormalTok{(}\StringTok{"number"}\NormalTok{, }\StringTok{"Enter a number"}\NormalTok{, }\AttributeTok{value =} \DecValTok{1}\NormalTok{, }\AttributeTok{min =} \DecValTok{1}\NormalTok{),}
  \FunctionTok{textOutput}\NormalTok{(}\StringTok{"square"}\NormalTok{),}
  \FunctionTok{textOutput}\NormalTok{(}\StringTok{"cube"}\NormalTok{)}
\NormalTok{)}

\NormalTok{server }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(input, output) \{}
  \CommentTok{\# Create a reactive expression to store the number}
\NormalTok{  number }\OtherTok{\textless{}{-}} \FunctionTok{reactive}\NormalTok{(\{}
\NormalTok{    input}\SpecialCharTok{$}\NormalTok{number}
\NormalTok{  \})}

\NormalTok{  output}\SpecialCharTok{$}\NormalTok{square }\OtherTok{\textless{}{-}} \FunctionTok{renderText}\NormalTok{(\{}
    \FunctionTok{paste}\NormalTok{(}\StringTok{"Square of the number:"}\NormalTok{, }\FunctionTok{number}\NormalTok{()}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{)}
\NormalTok{  \})}

\NormalTok{  output}\SpecialCharTok{$}\NormalTok{cube }\OtherTok{\textless{}{-}} \FunctionTok{renderText}\NormalTok{(\{}
    \FunctionTok{paste}\NormalTok{(}\StringTok{"Cube of the number:"}\NormalTok{, }\FunctionTok{number}\NormalTok{()}\SpecialCharTok{\^{}}\DecValTok{3}\NormalTok{)}
\NormalTok{  \})}
\NormalTok{\}}
...
\NormalTok{ui }\OtherTok{\textless{}{-}} \FunctionTok{fluidPage}\NormalTok{(}
  \FunctionTok{selectInput}\NormalTok{(}\StringTok{"gear"}\NormalTok{, }\StringTok{"Number of Gears"}\NormalTok{, }
              \AttributeTok{choices =} \FunctionTok{unique}\NormalTok{(mtcars}\SpecialCharTok{$}\NormalTok{gear), }
              \AttributeTok{selected =} \DecValTok{3}\NormalTok{),}
  \FunctionTok{actionButton}\NormalTok{(}\StringTok{"goButton"}\NormalTok{, }\StringTok{"Go!"}\NormalTok{),}
...
\NormalTok{    input}\SpecialCharTok{$}\NormalTok{goButton}

    \CommentTok{\# Use isolate() to avoid dependency on input$gear}
\NormalTok{    selected\_gear }\OtherTok{\textless{}{-}} \FunctionTok{isolate}\NormalTok{(input}\SpecialCharTok{$}\NormalTok{gear)}
\NormalTok{    gear\_data }\OtherTok{\textless{}{-}} \FunctionTok{subset}\NormalTok{(mtcars, gear }\SpecialCharTok{==}\NormalTok{ selected\_gear)}

    \CommentTok{\# Plot the polar coordinate plot of miles per gallon (mpg) for the selected number of gears}
\NormalTok{    p }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(gear\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =} \FunctionTok{factor}\NormalTok{(}\DecValTok{1}\NormalTok{), }\AttributeTok{y =}\NormalTok{ mpg, }\AttributeTok{fill =} \FunctionTok{factor}\NormalTok{(cyl))) }\SpecialCharTok{+}
      \FunctionTok{geom\_bar}\NormalTok{(}\AttributeTok{stat =} \StringTok{"identity"}\NormalTok{, }\AttributeTok{width =} \DecValTok{1}\NormalTok{) }\SpecialCharTok{+}
      \FunctionTok{coord\_polar}\NormalTok{(}\AttributeTok{theta =} \StringTok{"y"}\NormalTok{) }\SpecialCharTok{+}
      \FunctionTok{theme\_void}\NormalTok{() }\SpecialCharTok{+}
      \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =}\NormalTok{ stringr}\SpecialCharTok{::}\FunctionTok{str\_c}\NormalTok{(}\StringTok{"Polar Coordinate Plot for "}\NormalTok{, selected\_gear, }\StringTok{" Gears"}\NormalTok{),}
...
\hypertarget{reactive-values}{%
\section{Reactive Values}\label{reactive-values}}

\hypertarget{c.-create-a-shiny-app-with-increment-and-decrement-buttons-to-manipulate-a-counter-and-display-its-value-in-real-time-using-reactive-values-and-text-output.}{%
\subsection{c.~Create a Shiny app with increment and decrement buttons to manipulate a counter and display its value in real-time using reactive values and text output.}\label{c.-create-a-shiny-app-with-increment-and-decrement-buttons-to-manipulate-a-counter-and-display-its-value-in-real-time-using-reactive-values-and-text-output.}}
...
\NormalTok{ui }\OtherTok{\textless{}{-}} \FunctionTok{fluidPage}\NormalTok{(}
  \FunctionTok{actionButton}\NormalTok{(}\StringTok{"increment"}\NormalTok{, }\StringTok{"Increment"}\NormalTok{),}
  \FunctionTok{actionButton}\NormalTok{(}\StringTok{"decrement"}\NormalTok{, }\StringTok{"Decrement"}\NormalTok{),}
  \FunctionTok{textOutput}\NormalTok{(}\StringTok{"counter"}\NormalTok{)}
\NormalTok{)}

\NormalTok{server }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(input, output) \{}
  \CommentTok{\# Create a reactiveValues object to store the counter}
\NormalTok{  counter }\OtherTok{\textless{}{-}} \FunctionTok{reactiveValues}\NormalTok{(}\AttributeTok{value =} \DecValTok{0}\NormalTok{)}

  \FunctionTok{observeEvent}\NormalTok{(input}\SpecialCharTok{$}\NormalTok{increment, \{}
\NormalTok{    counter}\SpecialCharTok{$}\NormalTok{value }\OtherTok{\textless{}{-}}\NormalTok{ counter}\SpecialCharTok{$}\NormalTok{value }\SpecialCharTok{+} \DecValTok{1}
\NormalTok{  \})}

  \FunctionTok{observeEvent}\NormalTok{(input}\SpecialCharTok{$}\NormalTok{decrement, \{}
\NormalTok{    counter}\SpecialCharTok{$}\NormalTok{value }\OtherTok{\textless{}{-}}\NormalTok{ counter}\SpecialCharTok{$}\NormalTok{value }\SpecialCharTok{{-}} \DecValTok{1}
\NormalTok{  \})}

\NormalTok{  output}\SpecialCharTok{$}\NormalTok{counter }\OtherTok{\textless{}{-}} \FunctionTok{renderText}\NormalTok{(\{}
    \FunctionTok{paste}\NormalTok{(}\StringTok{"Counter value:"}\NormalTok{, counter}\SpecialCharTok{$}\NormalTok{value)}
\NormalTok{  \})}
\NormalTok{\}}
...
\begin{Highlighting}[]
\NormalTok{ui }\OtherTok{\textless{}{-}} \FunctionTok{fluidPage}\NormalTok{(}
  \AttributeTok{theme =}\NormalTok{ bslib}\SpecialCharTok{::}\FunctionTok{bs\_theme}\NormalTok{(}\AttributeTok{version =} \DecValTok{4}\NormalTok{, }\AttributeTok{bootswatch =} \StringTok{"flatly"}\NormalTok{),}
  \FunctionTok{titlePanel}\NormalTok{(}\StringTok{"Panel Plot Demo with Shiny and bslib"}\NormalTok{),}
  
  \FunctionTok{sidebarLayout}\NormalTok{(}
    \FunctionTok{sidebarPanel}\NormalTok{(}
      \FunctionTok{selectInput}\NormalTok{(}\StringTok{"var1"}\NormalTok{, }\StringTok{"Variable 1:"}\NormalTok{, }\AttributeTok{choices =} \FunctionTok{colnames}\NormalTok{(mtcars), }\AttributeTok{selected =} \StringTok{"mpg"}\NormalTok{),}
      \FunctionTok{selectInput}\NormalTok{(}\StringTok{"var2"}\NormalTok{, }\StringTok{"Variable 2:"}\NormalTok{, }\AttributeTok{choices =} \FunctionTok{colnames}\NormalTok{(mtcars), }\AttributeTok{selected =} \StringTok{"wt"}\NormalTok{),}
      \FunctionTok{actionButton}\NormalTok{(}\StringTok{"refresh"}\NormalTok{, }\StringTok{"Refresh Plot"}\NormalTok{)}
\NormalTok{    ),}
    
    \FunctionTok{mainPanel}\NormalTok{(}
      \FunctionTok{plotOutput}\NormalTok{(}\StringTok{"panelPlot"}\NormalTok{)}
\NormalTok{    )}
\NormalTok{  )}
\NormalTok{)}
...
\NormalTok{  output}\SpecialCharTok{$}\NormalTok{panelPlot }\OtherTok{\textless{}{-}} \FunctionTok{renderPlot}\NormalTok{(\{}
\NormalTok{    input}\SpecialCharTok{$}\NormalTok{refresh}
    
    \FunctionTok{isolate}\NormalTok{(\{}
\NormalTok{       panel\_plot }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(mtcars, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =} \SpecialCharTok{!!}\FunctionTok{sym}\NormalTok{(input}\SpecialCharTok{$}\NormalTok{var1), }\AttributeTok{y =} \SpecialCharTok{!!}\FunctionTok{sym}\NormalTok{(input}\SpecialCharTok{$}\NormalTok{var2))) }\SpecialCharTok{+}
        ...
        \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =}\NormalTok{ input}\SpecialCharTok{$}\NormalTok{var1, }\AttributeTok{y =}\NormalTok{ input}\SpecialCharTok{$}\NormalTok{var2, }\AttributeTok{color =} \StringTok{"Cylinders"}\NormalTok{) }\SpecialCharTok{+}
        \FunctionTok{theme}\NormalTok{(}\AttributeTok{legend.position =} \StringTok{"bottom"}\NormalTok{)}
...
\NormalTok{ui }\OtherTok{\textless{}{-}} \FunctionTok{fluidPage}\NormalTok{(}
  \FunctionTok{sliderInput}\NormalTok{(}\StringTok{"mpg\_threshold"}\NormalTok{, }\StringTok{"MPG Threshold"}\NormalTok{, }\FunctionTok{min}\NormalTok{(mtcars}\SpecialCharTok{$}\NormalTok{mpg), }\FunctionTok{max}\NormalTok{(mtcars}\SpecialCharTok{$}\NormalTok{mpg), }\AttributeTok{value =} \DecValTok{20}\NormalTok{, }\AttributeTok{step =} \FloatTok{0.5}\NormalTok{),}
  \FunctionTok{actionButton}\NormalTok{(}\StringTok{"goButton"}\NormalTok{, }\StringTok{"Go!"}\NormalTok{),}
  \FunctionTok{textOutput}\NormalTok{(}\StringTok{"num\_obs"}\NormalTok{)}
\NormalTok{)}

\NormalTok{server }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(input, output) \{}
  \CommentTok{\# Use eventReactive() to update the number of observations above the threshold}
\NormalTok{  num\_obs\_above\_threshold }\OtherTok{\textless{}{-}} \FunctionTok{eventReactive}\NormalTok{(input}\SpecialCharTok{$}\NormalTok{goButton, \{}
    \FunctionTok{sum}\NormalTok{(mtcars}\SpecialCharTok{$}\NormalTok{mpg }\SpecialCharTok{\textgreater{}}\NormalTok{ input}\SpecialCharTok{$}\NormalTok{mpg\_threshold)}
\NormalTok{  \})}

\NormalTok{  output}\SpecialCharTok{$}\NormalTok{num\_obs }\OtherTok{\textless{}{-}} \FunctionTok{renderText}\NormalTok{(\{}
    \FunctionTok{paste}\NormalTok{(}\StringTok{"Number of observations above the threshold:"}\NormalTok{, }\FunctionTok{num\_obs\_above\_threshold}\NormalTok{())}
\NormalTok{  \})}
\NormalTok{\}}
...
\hypertarget{more-with-isolate}{%
\subsection{\texorpdfstring{More with \texttt{isolate}}{More with isolate}}\label{more-with-isolate}}
...
\NormalTok{ui }\OtherTok{\textless{}{-}} \FunctionTok{fluidPage}\NormalTok{(}
  \AttributeTok{theme =} \FunctionTok{bs\_theme}\NormalTok{(}\AttributeTok{version =} \DecValTok{4}\NormalTok{, }\AttributeTok{bootswatch =} \StringTok{"flatly"}\NormalTok{),}
  \FunctionTok{titlePanel}\NormalTok{(}\StringTok{"Interactive Plot Demo with Shiny, bslib, and ggplot2"}\NormalTok{),}
...
\NormalTok{    input}\SpecialCharTok{$}\NormalTok{refresh}
    ...
    \FunctionTok{isolate}\NormalTok{(\{}
\NormalTok{      p }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data =}\NormalTok{ mtcars, }
                  \AttributeTok{mapping =} \FunctionTok{aes}\NormalTok{(}\AttributeTok{x =} \SpecialCharTok{!!}\FunctionTok{sym}\NormalTok{(input}\SpecialCharTok{$}\NormalTok{var1), }\AttributeTok{y =} \SpecialCharTok{!!}\FunctionTok{sym}\NormalTok{(input}\SpecialCharTok{$}\NormalTok{var2), }\AttributeTok{color =} \FunctionTok{factor}\NormalTok{(cyl))) }\SpecialCharTok{+}
        \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{size =} \DecValTok{3}\NormalTok{) }\SpecialCharTok{+}
        \FunctionTok{scale\_color\_viridis\_d}\NormalTok{(}\AttributeTok{option =} \StringTok{"viridis"}\NormalTok{, }\AttributeTok{name =} \StringTok{"Cylinders"}\NormalTok{) }\SpecialCharTok{+}
        \FunctionTok{theme\_minimal}\NormalTok{() }\SpecialCharTok{+}
        \FunctionTok{theme}\NormalTok{(}\AttributeTok{legend.position =} \StringTok{"bottom"}\NormalTok{) }\SpecialCharTok{+}
        \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =}\NormalTok{ input}\SpecialCharTok{$}\NormalTok{var1, }\AttributeTok{y =}\NormalTok{ input}\SpecialCharTok{$}\NormalTok{var2)}
...
\includegraphics[width=1\linewidth]{class_activity_21_files/figure-latex/unnamed-chunk-2-1}
...
\includegraphics[width=1\linewidth]{class_activity_22_files/figure-latex/unnamed-chunk-13-1}
...
# Groups:   .metric [4]
 ...
 1        23 accuracy binary     0.747    10  0.0253 Prepro~
 2        24 accuracy binary     0.747    10  0.0253 Prepro~
 3        21 ppv      binary     0.771    10  0.0190 Prepro~
 4        22 ppv      binary     0.771    10  0.0190 Prepro~
 5        23 sens     binary     0.875    10  0.0251 Prepro~
 6        24 sens     binary     0.875    10  0.0251 Prepro~
 7        37 sens     binary     0.875    10  0.0228 Prepro~
 8        38 sens     binary     0.875    10  0.0228 Prepro~
 9        39 sens     binary     0.875    10  0.0251 Prepro~
10        40 sens     binary     0.875    10  0.0251 Prepro~
11        21 spec     binary     0.531    10  0.0524 Prepro~
12        22 spec     binary     0.531    10  0.0524 Prepro~
\end{verbatim}
...
\includegraphics[width=1\linewidth]{class_activity_23_files/figure-latex/unnamed-chunk-9-1}
...
\includegraphics[width=1\linewidth]{class_activity_23_files/figure-latex/unnamed-chunk-13-1}
...
\includegraphics[width=1\linewidth]{class_activity_24_files/figure-latex/unnamed-chunk-4-1}
...
\includegraphics[width=1\linewidth]{class_activity_24_files/figure-latex/unnamed-chunk-6-1}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{multi\_kmeans }\OtherTok{\textless{}{-}} \FunctionTok{tibble}\NormalTok{(}\AttributeTok{k =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{10}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}
    \AttributeTok{model =}\NormalTok{ purrr}\SpecialCharTok{::}\FunctionTok{map}\NormalTok{(k, }\SpecialCharTok{\textasciitilde{}} \FunctionTok{kmeans}\NormalTok{(my\_df, }\AttributeTok{centers =}\NormalTok{ .x, }\AttributeTok{nstart =} \DecValTok{25}\NormalTok{)),}
    \AttributeTok{tot.withinss =}\NormalTok{ purrr}\SpecialCharTok{::}\FunctionTok{map\_dbl}\NormalTok{(model, }\SpecialCharTok{\textasciitilde{}} \FunctionTok{glance}\NormalTok{(.x)}\SpecialCharTok{$}\NormalTok{tot.withinss)}
\NormalTok{  )}
\end{Highlighting}
\end{Shaded}
...
\includegraphics[width=1\linewidth]{class_activity_24_files/figure-latex/unnamed-chunk-8-1}
...
\includegraphics[width=1\linewidth]{class_activity_24_files/figure-latex/unnamed-chunk-9-1}
...
\includegraphics[width=1\linewidth]{class_activity_24_files/figure-latex/unnamed-chunk-10-1}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{rpart.plot}\NormalTok{(tree\_fit}\SpecialCharTok{$}\NormalTok{fit, }\AttributeTok{roundint =} \ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics[width=1\linewidth]{class_activity_25_files/figure-latex/unnamed-chunk-13-1}
...
\includegraphics[width=1\linewidth]{class_activity_25_files/figure-latex/unnamed-chunk-20-1}
...
\includegraphics[width=1\linewidth]{class_activity_27_files/figure-latex/unnamed-chunk-11-1}
